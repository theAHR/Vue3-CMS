...
import FaqCategoryDialog from '@/components/common/FaqCategoryDialog.vue';
import ConfirmDialog from '@/components/common/ConfirmDialog.vue';
import DetailsDialog from '@/components/common/DetailsDialog.vue';
import Button from '@/components/common/Button.vue';
import { useFaqCategoryStore } from '@/stores';
...
const {
  ...
  showDetailsDialog,
  selectedCategory
} = storeToRefs(faqCategoryStore);

const deleteConfirmMessage = computed(() => {
  if (!categoryToDelete.value) return 'آیا از حذف این دسته‌بندی اطمینان دارید؟';
  return `آیا از حذف دسته‌بندی "${categoryToDelete.value.displayName}" اطمینان دارید؟`;
});

const tableColumns = [
  { key: 'rowNumber', label: 'ردیف', class: 'text-center' },
  { key: 'displayName', label: 'عنوان', class: 'text-center' },
...
const handleLoadMore = async () => {
  try {
    await faqCategoryStore.loadMoreCategories();
  } catch (err) {
...
const showAddDialog = () => {
  faqCategoryStore.openAddDialog();
};

const closeDialog = () => {
  faqCategoryStore.closeDialog();
};

const handleEdit = (category) => {
  faqCategoryStore.openEditDialog(category);
};

const handleDetails = (category) => {
  faqCategoryStore.openDetailsDialog(category);
};

const handleDelete = (category) => {
  faqCategoryStore.openDeleteConfirm(category);
};

const confirmDelete = async () => {
  try {
    await faqCategoryStore.deleteCategory(categoryToDelete.value.id);
    success('دسته‌بندی با موفقیت حذف شد');
    faqCategoryStore.closeDeleteConfirm();
  } catch (err) {
...
const handleSearch = async (query) => {
  faqCategoryStore.setSearchQuery(query);
  faqCategoryStore.setCurrentPage(1);
  try {
    await faqCategoryStore.searchCategories();
  } catch (err) {
...
const handleSubmit = async (formData) => {
  try {
    let response;
    if (formData.id) {
      response = await faqCategoryStore.updateCategory(formData);
    } else {
      response = await faqCategoryStore.createCategory(formData);
    }
